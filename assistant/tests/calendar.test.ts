...
describe('calendar merge', () => {
  it('orders events from both providers by time', () => {
    ...
    const outlook = [
      ev({ provider: 'microsoft', calendarName: 'Work', title: 'הרצאה', start: '2026-09-10T08:30:00Z', end: '2026-09-10T09:30:00Z' }),
      ev({ provider: 'microsoft', calendarName: 'Work', title: 'רופא', start: '2026-09-10T14:00:00Z', end: '2026-09-10T14:45:00Z' }),
    ];
  ...
  it('deduplicates by iCalUID across providers', () => {
    ...
    const merged = mergeCalendars([
      [ev({ title: 'Standup', icalUid: shared, start: '2026-09-10T06:00:00Z', end: '2026-09-10T06:15:00Z' })],
      [ev({ provider: 'microsoft', calendarName: 'Work', title: 'Standup', icalUid: shared, start: '2026-09-10T06:00:00Z', end: '2026-09-10T06:15:00Z' })],
    ]);
  ...
  it('deduplicates identical events even without a shared UID', () => {
    const merged = mergeCalendars([
      ...
      [ev({ provider: 'microsoft', calendarName: 'Work', title: 'פגישת צוות', start: '2026-09-10T07:00:00Z', end: '2026-09-10T08:00:00Z' })],
    ]);
    expect(merged).toHaveLength(1);
  ...
  it('drops cancelled events', () => {
    const merged = mergeCalendars([
      [ev({ title: 'בוטל', isCancelled: true, start: '2026-09-10T07:00:00Z', end: '2026-09-10T08:00:00Z' })],
    ]);
...
describe('busy periods', () => {
  ...
  it('ignores events marked free', () => {
    expect(mergeBusy([ev({ title: 'free', showAsBusy: false, start: '2026-09-10T07:00:00Z', end: '2026-09-10T08:00:00Z' })])).toHaveLength(0);
  });

  it('ignores all-day markers', () => {
    expect(mergeBusy([ev({ title: 'חופש', allDay: true, start: '2026-09-10T00:00:00Z', end: '2026-09-11T00:00:00Z' })])).toHaveLength(0);
  });
});
...
describe('free slots', () => {
  ...
  const events = [
    ...
    ev({ provider: 'microsoft', calendarName: 'W', title: 'סדנה', start: '2026-09-10T09:00:00Z', end: '2026-09-10T11:00:00Z' }),
  ...
  it('finds the gaps inside the working day', () => {
    const slots = computeFreeSlots(events, {
      date: '2026-09-10', timezone: TZ, dayStart: '09:00', dayEnd: '18:00', minMinutes: 60,
    });
    ...
    expect(asLocal).toEqual([['07:00', '09:00'], ['11:00', '13:00'], ['14:00', '15:00']]);
  });

  it('honours the minimum slot length', () => {
    const slots = computeFreeSlots(events, {
      date: '2026-09-10', timezone: TZ, dayStart: '09:00', dayEnd: '18:00', minMinutes: 120,
    });
  ...
  it('returns nothing when the day is fully booked', () => {
    const full = [ev({ title: 'כל היום', start: '2026-09-10T06:00:00Z', end: '2026-09-10T15:00:00Z' })];
    expect(computeFreeSlots(full, {
      date: '2026-09-10', timezone: TZ, dayStart: '09:00', dayEnd: '18:00', minMinutes: 30,
    })).toHaveLength(0);
  });
});

describe('conflict detection', () => {
  const events = [ev({ title: 'פגישה קיימת', start: '2026-09-13T10:00:00Z', end: '2026-09-13T11:00:00Z' })];

  it('detects an overlapping proposal', () => {
    const conflicts = findConflicts(events, {
      start: new Date('2026-09-13T10:30:00Z'), end: new Date('2026-09-13T11:30:00Z'),
    });
  ...
  it('allows a back-to-back slot', () => {
    const conflicts = findConflicts(events, {
      start: new Date('2026-09-13T11:00:00Z'), end: new Date('2026-09-13T12:00:00Z'),
    });
